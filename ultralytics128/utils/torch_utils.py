...
from ultralytics128.utils.cpu import CPUInfo
...
def get_cpu_info():
    ...
    from ultralytics128.utils import PERSISTENT_CACHE  # avoid circular import error
...
def select_device(device="", newline=False, verbose=True):
    ...
    # Auto-select GPUs
    if "-1" in device:
        from ultralytics128.utils.autodevice import GPUInfo
...
def strip_optimizer(f: str | Path = "best.pt", s: str = "", updates: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    Examples:
        >>> from pathlib import Path
        >>> from ultralytics128.utils.torch_utils import strip_optimizer
...
def profile_ops(input, ops, n=10, device=None, max_num_obj=0):
    ...
    Examples:
        >>> from ultralytics128.utils.torch_utils import profile_ops
